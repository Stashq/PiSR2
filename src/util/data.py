...
def get_train_test_ratings(ratings: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    train_ratings, test_ratings = train_test_split(
        ratings, test_size=0.1, stratify=ratings["movieId"].values, random_state=42
    )
...
def get_interactions(
    ratings: pd.DataFrame, user_encoder: LabelEncoder, movie_encoder: LabelEncoder
) -> np.ndarray:
    """
