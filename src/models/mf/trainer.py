...
def get_metrics(model, dataloader, mse_loss, max_samples=-1, device="cpu"):
    ...
    if loss_elems > 0:
        loss /= loss_elems
    return {"RMSE": loss ** 0.5, "Loss": loss}
    ...
    with torch.utils.tensorboard.SummaryWriter(comment=comment) as writer:
        for epoch in pbar1:  # loop over the dataset multiple times
            ...
            for i, (users, movies, ratings) in enumerate(train_loader):
                ...
                for reg_param, param in zip(regularizing_params, model.parameters()):
                ...
                writer.add_scalar("batch/MSELoss", loss.item(), tensorboard_step)
                writer.add_scalar("batch/l2_reg", l2_reg.item(), tensorboard_step)
                pbar2.set_postfix({"batch loss": loss.item()})
                pbar2.update(1)

            pbar1.set_description("metrics on train_df...")
            metrics = get_metrics(
                model, train_loader, criterion, max_samples=500, device=device
            )
            ...
            if test_loader is not None:
                ...
                metrics = get_metrics(model, test_loader, criterion, device=device)
                pbar1.set_postfix(metrics)
                for metric_name, metric_value in metrics.items():
                    writer.add_scalar(f"{metric_name}/validation", metric_value, epoch)
    return metrics
