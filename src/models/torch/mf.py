...
class MatrixFactorization(nn.Module, Recommender):
    ...
    def initialize(self, pandas_dataframe_reindexed, embedding_rescaler=1.0):
        ...
        self.user_bias.weight.data = \
            torch.full(self.user_bias.weight.shape, 0.0)
        self.movie_bias.weight.data = \
            torch.full(self.movie_bias.weight.shape, 0.0)

        movie_stats = pandas_dataframe_reindexed.groupby('movieId')\
            .agg({'rating':['mean']})
        movie_stats.columns = movie_stats.columns.get_level_values(1)

        user_stats = pandas_dataframe_reindexed.groupby('userId')\
            .agg({'rating':['mean']})
        user_stats.columns = user_stats.columns.get_level_values(1)

        for user_id, user_mean in zip(user_stats['mean'].index,
                                      user_stats['mean']):
            self.user_bias.weight.data[user_id,:] = user_mean - mean_rating

        for movie_id, movie_mean in zip(movie_stats['mean'].index,
                                        movie_stats['mean']):
    ...
    def assert_encoders_exist(self):
        assert ((self.user_encoder is not None) 
    ...
    def predict_scores(self, user_id: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        movies_seen = \
            self.pandas_dataframe_reindexed[
