...
def get_dataloaders(train_ratings, test_ratings, 
    ...
    test_ratings_reindexed['userId'] = \
        user_encoder.transform(test_ratings_reindexed['userId'].values)
    test_ratings_reindexed['movieId'] = \
        movie_encoder.transform(test_ratings_reindexed['movieId'].values)
    
    #torch datasets
    train_dataset = torch.utils.data.TensorDataset(
        torch.tensor(train_ratings_reindexed['userId'].values),
        torch.tensor(train_ratings_reindexed['movieId'].values),
        torch.tensor(train_ratings_reindexed['rating'].values,
                     dtype=torch.float),
        )
    ...
    test_dataset = torch.utils.data.TensorDataset(
        ...
        torch.tensor(test_ratings_reindexed['rating'].values,
                     dtype=torch.float),
        )
